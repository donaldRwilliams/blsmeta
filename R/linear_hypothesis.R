...
print.linear_hypothesis <- function(x, ...) {
  ...
  cri_raw <- bayeslincom:::extract_list_items(res, "ci")
  cri <- round(cri_raw, 2)
  
  Post.mean_raw <- bayeslincom:::extract_list_items(res, "mean_samples")
  Post.mean <- round(Post.mean_raw, 2)
  
  Post.sd_raw <- bayeslincom:::extract_list_items(res, "sd_samples")
  ...
  cat("Hypotheses:\n")
  comb_list <- bayeslincom:::extract_list_items(res, "lin_comb")
  ...
  if (!is.null(x$rope)) {
    ...
    print_df$Pr.in <- bayeslincom:::extract_list_items(res, "rope_overlap")
    
    # note for ROPE
    note <- "Pr.in: Posterior probability in ROPE"
  } else {
    prob_greater <- bayeslincom:::extract_list_items(res, "prob_greater")
